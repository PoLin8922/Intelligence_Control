...
class KernelPerceptron(object):
    ...
    def fit(self, X, y):
        n_samples, n_features = X.shape  # (180,2)
        ...
        for t in range(self.T):
            for i in range(n_samples):
                if np.sign(np.sum(K[:,i] * self.alpha * y)) != y[i]:  # K[:,i] is column i for K
                    self.alpha[i] += 1.0
...
def test_kernel():
    ...
    clf = KernelPerceptron(gaussian_kernel, T=100)
    clf.fit(X_train, y_train)

    y_predict = clf.predict(X_test)
    correct = np.sum(y_predict == y_test)
    a = float(correct)/len(y_predict)
    print("accuracy: %s" % (a))
